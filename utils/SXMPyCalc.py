...
class CITSCalculator:
    ...
    def calculate_cits_coordinates(
        ...
        try:
            ...
            # 計算實際掃描範圍
            fast_axis_range = scan_range * (1 - safe_margin)
            slow_axis_range = fast_axis_range / aspect_ratio  # 當 aspect_ratio < 1 時慢軸範圍變大
            ...
            scanlines = CITSCalculator.calculate_scanline_distribution(
                total_scan_lines, num_points_y, 0.004
            )
...
class LocalCITSCalculator:
    ...
    def get_scan_axes(scan_angle: float, scan_direction: int) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # 慢軸：慢軸逆時針旋轉90度
        slow_axis = np.array(
            [-np.sin(angle_rad), np.cos(angle_rad)])*scan_direction
    ...
    def combi_local_cits_coordinates(
        ...
        """
        # 獲取快慢軸方向
        slow_axis, fast_axis = LocalCITSCalculator.get_scan_axes(
    ...
    def calculate_local_scanline_distribution(
        ...
        try:
            ...
            # 在返回結果之前，執行驗證檢查
            # 1. 檢查點數總和
            total_coords_in_distribution = sum(
                len(coords) for coords in coordinate_distribution)
