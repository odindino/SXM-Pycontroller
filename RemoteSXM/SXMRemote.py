...
# import SXMRemote  # 25/01/2021
import ctypes
import threading
import time
# import win32event    # this function is used for 32 bit Python, only
...
class DDE(object):
    ...
    AccessData = get_winfunc("user32", "DdeAccessData",
                             LPBYTE,   (HDDEDATA, LPDWORD))
    ClientTransaction = get_winfunc("user32", "DdeClientTransaction",   HDDEDATA, (
        LPBYTE, DWORD, HCONV, HSZ, UINT, UINT, DWORD, LPDWORD))
    Connect = get_winfunc("user32", "DdeConnect",
                          HCONV,    (DWORD, HSZ, HSZ, PCONVCONTEXT))
    CreateStringHandle = get_winfunc(
        "user32", "DdeCreateStringHandleW", HSZ,      (DWORD, LPCWSTR, UINT))
    Disconnect = get_winfunc("user32", "DdeDisconnect",
                             BOOL,     (HCONV,))
    GetLastError = get_winfunc(
        "user32", "DdeGetLastError",        UINT,     (DWORD,))
    Initialize = get_winfunc("user32", "DdeInitializeW",
                             UINT,     (LPDWORD, DDECALLBACK, DWORD, DWORD))
    ...
    QueryString = get_winfunc("user32", "DdeQueryStringA",
                              DWORD,    (DWORD, HSZ, LPSTR, DWORD, c_int))
...
class DDEClient(object):  # 'self.' in whole class
    ...
    def __init__(self, service, topic):
        ...
        res = DDE.Initialize(byref(self._idInst),
                             self._callback, 0x00000010, 0)
        ...
        if not self._hConv:
            raise DDEError(
                "Unable to establish a conversation with server", self._idInst)
    ...
    def execute(self, command, timeout=5000):
        ...
        hDdeData = DDE.ClientTransaction(
            pData, cbData, self._hConv, HSZ(), CF_TEXT, XTYP_EXECUTE, timeout, LPDWORD())
        if not hDdeData:
    ...
    def request(self, item, timeout=5000):
        ...
        hszItem = DDE.CreateStringHandle(self._idInst, item, 1200)
        hDdeData = DDE.ClientTransaction(
            LPBYTE(), 0, self._hConv, hszItem, CF_TEXT, XTYP_REQUEST, timeout, LPDWORD())
        DDE.FreeStringHandle(self._idInst, hszItem)
        if not hDdeData:
    ...
    def Scan(self, LineNr):
        # print(LineNr); # 1st letter is u/d/f/b for up/down/forward/backward
        pass
...
def loop():
    ...
    LPMSG = POINTER(MSG)
    LRESULT = c_ulong
    GetMessage = get_winfunc("user32", "GetMessageW",
                             BOOL, (LPMSG, HWND, UINT, UINT))
    TranslateMessage = get_winfunc(
        "user32", "TranslateMessage", BOOL, (LPMSG,))
    # restype = LRESULT
    DispatchMessage = get_winfunc(
...
# Do not execute here. Otherwise the class will be instantiated twice
# and functins will run twice (25/01/21)
